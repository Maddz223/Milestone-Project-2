...
const TVDetails = () => {
  ...
  useEffect(() => {
    ...
    {/* read results and import into Tv Details page */ }
    fetchTvDetails();
  }, [tvId]);

  // Update watchlist state when tvDetails load
  useEffect(() => {
    if (tvDetails) {
      ...
      <div className="flex flex-col lg:flex-row items-center lg:items-start gap-6 rounded-lg bg-gray-100 dark:bg-gray-900 p-6">
        ...
        <div className="flex-1 text-center lg:text-left space-y-4">
          ...
          <div>
            ...
            {/* Trailer Modal */}
            {selectedTrailer && <TrailerModal trailer={selectedTrailer} onClose={() => setSelectedTrailer(null)} />}
          </div>
              ...
              Object.entries(watchProviders).map(([country, data]) => (
                <div key={country} className="flex justify-center flex-wrap gap-4 mt-2">
                  {Array.isArray(data.flatrate) && data.flatrate.length > 0 ? (
                    ...
                    <span className="text-sm text-gray-500 w-full text-center">No streaming providers available.</span>
                  )}
                </div>
              ))
            ) : (
